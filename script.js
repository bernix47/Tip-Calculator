...
function calculateTip(){
    ...
    let noOfPeople = parseFloat(document.getElementById("no-of-people").value);

    billInput.value = bill.toFixed(2);

    let totalTip = parseFloat((bill * (tipPercent/100)).toFixed(2));
    let total = parseFloat((bill + totalTip).toFixed(2));
    let tipPerPerson = Math.ceil((totalTip / noOfPeople).toFixed(2));
    let totalPerPerson = Math.ceil((total / noOfPeople).toFixed(2));
    console.log(total,totalTip,tipPerPerson,totalPerPerson,bill,tipPercent,noOfPeople);
